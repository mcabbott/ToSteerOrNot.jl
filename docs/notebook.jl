...
# ╔═╡ 166bbf09-cfb6-4d5b-8d04-a919b7dabf44
using ToSteerOrNot # added as here: https://plutojl.org/en/docs/packages-advanced/#activate_notebook_environment

# ╔═╡ 32d3197c-6471-11f1-3d55-01e2fe2ab269
using Plots, DelimitedFiles

# ╔═╡ 5091c447-af75-4654-8a15-a155f57e1d3a
md"""
...
This is a reactive notebook. When running locally (or in the cloud), editing any cell and pressing shift-enter will update other cells as needed. You can, for instance, edit values of `gamma`, and then see how the plots update.

`using Plots, DelimitedFiles` loads a package from the public registry, and a built-in standard library. `using ToSteerOrNot` loads this package, which is not registered, but had beed added to this notebook using its github web address. The complete list of all downstream packages needed is stored as part of the notebook.

Pluto is opinionated. In particular, outputs are always printed above the code cells! Every code cell must be one statement. And nothing may be defined twice.
"""

# ╔═╡ 088cd0d0-4e0d-457f-be43-9d1568b51ce1
x = 1+2

# ╔═╡ 42da05ba-b525-4c6d-b799-b39522201e80
y = 10*x

# ╔═╡ 5dac8be1-0022-4e10-bf59-9f79645b7476
string("x+y is now ", x+y)

# ╔═╡ fa3c5ad2-5156-4ffb-ae6b-2c3d11ce834c
Plots.default(titlefontsize=12)

# ╔═╡ 64f2a12c-c6d4-4d3a-b717-9d65eb273703
md"""
...
# ╔═╡ b943d226-4fa2-4560-bf1c-4d21af9f870a
begin
...
# ╔═╡ 624973b8-a800-4655-8069-623ede346981
let sol = rev01
...
# ╔═╡ 712b2d70-174d-4e2e-a96a-0387d70e4470
let sol = ste01
...
# ╔═╡ 79305f60-66a2-46ab-9ef5-db8d19d90a91
let sol = abs01
...
# ╔═╡ f267aeb6-d12d-49c8-8548-aeb3fa54871a
md"""
...
# ╔═╡ 792ed7e5-682b-41e2-a98d-6b4539ef78c9
tumble40 = [max2Wcostumble(γ) for γ in logrange(0.001, 2, 40)]

# ╔═╡ 8593b568-3031-46e9-bfd5-2a32b9ba2c39
reverse30 = [max2Wcosreverse(γ) for γ in logrange(0.001, 1.5, 30)]

# ╔═╡ b03a411f-b7e7-46a6-9c2a-29dc6542822a
steer40 = [max2Wsinsteer(γ) for γ in logrange(0.001, 2, 40)]

# ╔═╡ 51c64cd1-bb5d-42d4-aa50-b4dc989990d7
# ╠═╡ show_logs = false
evensteerd40 = [max2Wcosabs(γ) for γ in logrange(0.0005, 2, 40)]

# ╔═╡ 6a66bf01-edf1-4315-99b0-b124fd437840
# steer_exact = DelimitedFiles.readdlm("Analytic_Frontier_Points.csv", ' ')
steer_exact = DelimitedFiles.readdlm(joinpath(pkgdir(ToSteerOrNot), "docs/Analytic_Frontier_Points.csv"), ' ')  # more robust!

# ╔═╡ 18205c02-a952-4565-8579-73bdf5402cc8
...
# ╔═╡ bf50835f-906c-4a4c-bb34-38d9283404ee
let
...
# ╔═╡ 13a03b44-986d-48cf-95be-c577869a14cd
md"""
...
# ╔═╡ 6352f217-b2ec-4c95-a90a-e29808e59d95
simple_sym = max2Wjump(0.05, 36; signed=false, mirror=true)  # impose symmetry

# ╔═╡ 1c863360-5bcd-4b3a-892a-27a08b652e6d
...
# ╔═╡ 177eb5ae-d5a2-4907-8fb9-69d7d9c4adbe
...
# ╔═╡ 6618b4c7-cd80-417b-be41-1a18db0f4c97
...
ToSteerOrNot = "c95c5c81-f4ef-4917-a0cf-ca52a663e07c"

[sources.ToSteerOrNot]
rev = "master"
url = "https://github.com/mcabbott/ToSteerOrNot.jl"
...
manifest_format = "2.0"
project_hash = "2a9196415176e331ec7b22264045583771939c9c"
...
[[deps.ToSteerOrNot]]
deps = ["DifferentiableEigen", "FastGaussQuadrature", "ForwardDiff", "IterativeSolvers", "LinearAlgebra", "LinearMaps", "Optim", "Plots", "Random", "Statistics"]
git-tree-sha1 = "04bf89770931b5e341dfa8e9c18cf085cf9d8192"
repo-rev = "master"
repo-url = "https://github.com/mcabbott/ToSteerOrNot.jl"
uuid = "c95c5c81-f4ef-4917-a0cf-ca52a663e07c"
version = "0.1.0"
